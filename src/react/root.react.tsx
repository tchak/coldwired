...
export function createAndRenderReactRoot({
  ...
  container: Element;
  subscribe: (callback: (this: void) => void) => (this: void) => void;
  getSnapshot: (this: void) => Map<Element, ReactNode>;
...
const DefaultErrorBoundaryFallbackComponent: ErrorBoundaryFallbackComponent = ({
  ...
  const message = element.getAttribute('fallback-message') ?? (error as Error).message;
  return (
    <div role="alert">
